...
lines <- c(7,9,10,11,12,13,14,16,17,18,19,20)
...
sortByBenchmark <- function(data) {
  sysmark_benchs = isFromBench(data, c("IE", "word", "finereader", "powerpoint")) 
...
genGraph <- function(data,X, Y, FILL, XLAB, YLAB, HLINE, VLINE = TRUE, ANNOT = TRUE) {
  ...
  if (ANNOT)
    p <- p + annotate("text", x = 1.5, y = max(Y), label = "SPEC") + 
             annotate("text", x = 15.5, y = max(Y), label = "Sysmark")
...
genAndSaveAllGraphs <- function(data, PREFIX, GENFUNC) {
  for (i in lines) {
    p <- GENFUNC(data, i)
